    ...
    var body: some View {
        Canvas { context, canvasSize in
            ...
            if lineWidth > 2 {
                ...
                context.fill(highlightPath, with: .color(color.opacity(0.3)))
            }
        }
...
struct FluidIconFilled: View {
    ...
    var body: some View {
        ZStack {
            ...
            FluidIcon(size: size * 0.6, lineWidth: size * 0.08, color: color)
        }
    }
}
...
struct FluidIconAdvanced: View {
    ...
    var body: some View {
        Canvas { context, canvasSize in
            let rect = CGRect(origin: .zero, size: canvasSize)
            let workingRect = rect.insetBy(dx: size * 0.1, dy: size * 0.1)
